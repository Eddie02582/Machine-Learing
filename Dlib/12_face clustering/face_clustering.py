...
for index,num_classes in enumerate(labels):    
    classes_dict[num_classes].append(index)
    
print("Number of clusters: {}".format(classes_dict.keys()))

print (classes_dict)

biggest_class = max(classes_dict,key = lambda x:len(classes_dict[x]))
biggest_class_length = len(classes_dict[biggest_class])
...
indices = classes_dict[biggest_class]
...
print("Saving cluster faces to output folder...")

for num_classes,image_indexs in classes_dict.items():   
    output_folder_classes_path = os.path.join(output_folder_path,str(num_classes))
    if not os.path.isdir(output_folder_classes_path):
        os.makedirs(output_folder_classes_path)
    for i,image_index in enumerate(image_indexs):
        img, shape = images[image_index]
        file_path = os.path.join(output_folder_classes_path, "face_" + str(i))
